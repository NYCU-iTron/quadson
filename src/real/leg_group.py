...
class LegGroup:
    def __init__(self, leg_id: int, motor_manager: MotorManager):
        ...
        self.leg_id = leg_id
        self.motor_dict = {
            0: 3 * leg_id + 0,  # Motor 0
            1: 3 * leg_id + 1,  # Motor 1
            2: 3 * leg_id + 2   # Motor 2
        }
    ...
    def get_zero_states(self) -> list[bool]:
        zero_states = []
        for motor_id in self.motor_dict.values():
    ...
    def set_motor_angles(self, motor_angles: list[float]) -> None:
        for motor_id in self.motor_dict.values():
            self.motor_manager.set_motor_angle(motor_id, motor_angles[motor_id])

    def set_motor_omegas(self, motor_omegas: list[float]) -> None:
        for motor_id in self.motor_dict.values():
            self.motor_manager.set_motor_omega(motor_id, motor_omegas[motor_id])
    ...
    def set_ee_point(self, ee_point: list[float]) -> None:
        ...
        for motor_id in self.motor_dict.values():
            self.motor_manager.set_motor_omega(motor_id, motor_angles[motor_id])
